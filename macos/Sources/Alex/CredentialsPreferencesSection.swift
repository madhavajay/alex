...
struct CredentialsPreferencesSection: View {
    ...
    private func mintKey() async {
        ...
        do {
            mintedKey = try await AlexClient(config: config).mintRunKey(
    ...
    private func mintConnectKey() async {
        ...
        do {
            connectKey = try await AlexClient(config: config).mintRunKey(
    ...
    private func revoke(_ key: CredentialRunKey) async {
        ...
        do {
            try await AlexClient(config: config).revokeRunKey(id: key.id)
    ...
    private func revokeAll() async {
        ...
        do {
            let count = try await AlexClient(config: config).revokeAllRunKeys(includeHarness: true)
    ...
    private func clearRevoked() async {
        ...
        do {
            let count = try await AlexClient(config: config).clearRevokedRunKeys()
