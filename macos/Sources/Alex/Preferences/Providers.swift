...
struct ProvidersPreferencesSection: View {
    ...
    private func addAccount(_ provider: String) {
        ...
        case "exo", "openrouter":
            ...
            selectedProvider = provider
        default:
            // Subscription providers derive identity after browser/device
            // authentication. New upstream identities are added; signing into
            // the same identity replaces that account's credentials.
            onAuthenticate(provider, nil, true, false)
        }
    }
    ...
    private func loadEarliestActivity() async {
        ...
        do {
            let response = try await AlexClient(config: config)
...
private struct ProviderPreferencesDetail: View {
    ...
    private var showsAddAccount: Bool { true }
...
private struct SubscriptionAccountRow: View {
    ...
    private func setPaused(_ paused: Bool) {
        ...
        Task {
            do {
                try await AlexClient(config: config).setAccountPaused(id: account.id, paused: paused)
                await store.refresh()
            } catch {
    ...
    private func remove() {
        ...
        Task {
            do {
                try await AlexClient(config: config).removeAccount(id: account.id)
                await store.refresh()
            } catch {
                self.error = error.localizedDescription
            }
    ...
    private func reauthenticateViaNotification(force: Bool) {
        ...
        Task {
            do {
                let response = try await AlexClient(config: config).reauthNotify(
                    provider: account.provider, accountId: account.id, force: force)
                if let loginId = response.loginId,
                   let session = try? await AlexClient(config: config)
    ...
    private func submitPendingCode(_ session: LoginSession) {
        ...
        Task {
            do {
                let updated = try await AlexClient(config: config)
    ...
    private func resetPendingSession() {
        ...
        Task {
            do {
                ...
                let session = try await AlexClient(config: config).authLoginStatus(id: id)
                store.rememberLoginSession(
                    session, accountId: account.id, provider: account.provider)
                pendingCode = ""
    ...
    private func pollPendingSession() async {
        ...
        while !Task.isCancelled {
            ...
            do {
                let updated = try await AlexClient(config: config).authLoginStatus(id: id)
                store.rememberLoginSession(
                    updated, accountId: account.id, provider: account.provider)
...
private struct ProviderRoutingPreferencesSection: View {
    ...
    private func save() {
        ...
        Task {
            do {
                try await AlexClient(config: config).updateRouting(provider: provider, update)
                savedSignature = currentSignature
                await store.refresh()
            } catch {
                self.error = error.localizedDescription
