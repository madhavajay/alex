...
public final class SnapshotStore {
    ...
    public func refresh() async {
        ...
        let client = AlexClient(config: cfg)
    ...
    public func refreshHarnesses(using client: AlexClient) async {
        while refreshing {
    ...
    public func refreshHarnessesIfStale() async {
        ...
        do {
            if let fetched = try await AlexClient(config: config).harnessSnapshot(refresh: true) {
                harnesses = fetched.harnesses
                harnessesCheckedMs = fetched.checkedMs
                harnessesSupported = true
            }
