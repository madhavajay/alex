...
@Suite struct ModelDecodingTests {
    ...
    @Test func daemonUpdateStatus() throws {
        let json = #"""
        {"current":"0.1.0","latest":"0.2.0","update_available":true,"update_channel":"beta","notes_url":"https://example.test/notes","checked_at_ms":1783477427269}
        """#
        ...
        #expect(update.updateChannel == "beta")
    ...
    @Test func daemonUpdateApplyResponse() throws {
        let json = #"""
        {"applying":false,"current":"0.1.0","latest":"0.2.0","update_available":true,"update_channel":"stable","reason":"alex is managed by Homebrew - run `brew upgrade alex`"}
        """#
        let update = try decode(json, as: DaemonUpdateApplyResponse.self)
        #expect(!update.applying)
        #expect(update.updateAvailable == true)
        #expect(update.updateChannel == "stable")
    ...
    @Test func harnesses() throws {
        ...
        #expect(harnesses[1].backupPath?.hasSuffix("alex-original-config.toml") == true)
    }
    ...
    @Test func harnessPlanResponse() throws {
        let json = #"""
        {"plan":[
          {"path":"/Users/x/.pi/agent/models.json","action":"create","detail":"add provider 'alex' with 28 models"},
    ...
    @Test func alexErrorSessionOnlyExposesKnownPendingCredentialForApproval() throws {
        let pending = try decode(
            #"{"session_id":"alex-error-auth-abc","first_ts_ms":1,"last_ts_ms":2,"trace_count":1,"tags":{"alex_error":"true","alex_error_kind":"auth","approval_state":"pending","credential_fingerprint":"0000000000000001"}}"#,
            as: TraceSession.self)
        #expect(pending.isAlexError)
        #expect(pending.approvableCredentialFingerprint == "0000000000000001")
        #expect(SessionRow(session: pending).isAlexError)

        let unknown = try decode(
            #"{"session_id":"alex-error-auth-unknown","first_ts_ms":1,"last_ts_ms":2,"trace_count":1,"tags":{"alex_error":"true","approval_state":"unavailable","credential_fingerprint":"0000000000000002"}}"#,
            as: TraceSession.self)
