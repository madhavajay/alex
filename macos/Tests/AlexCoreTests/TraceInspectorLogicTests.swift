...
@Suite struct TraceInspectorLogicTests {
    ...
    @Test func documentMergesRequestedAndExecutedToolIntoOneCard() throws {
        ...
        #expect(toolLinks == [
            "alex://tool/exec-1/args",
            "alex://tool/exec-1/result",
        ])
    }
    ...
    @Test func traceLinkRoundtrip() {
        ...
        #expect(url?.absoluteString == "alex://trace/\(id)")
        #expect(url.flatMap(TraceLink.traceId(from:)) == id)
        #expect(TraceLink.url(forTraceId: "") == nil)
        #expect(TraceLink.traceId(from: URL(string: "alex://trace/")!) == nil)
        #expect(TraceLink.traceId(from: URL(string: "alex://other/abc")!) == nil)
