...
fn load() -> Result<Film> {
    ...
    for _ in 0..frames {
        ...
        let info = decoder
            .info()
            .ok_or_else(|| anyhow::anyhow!("no jpeg info"))?;
        if w == 0 {
...
#[allow(clippy::too_many_arguments)]
fn render_kitty(film: &Film, payload: &str, cols: usize, rows: usize, status: &[String]) -> String {
    let text_rows = if rows >= 18 { status.len().min(3) } else { 0 };
    let max_r = rows.saturating_sub(1 + text_rows).max(3);
    let mut c = cols.max(10);
    let mut r = ((c as f64) * film.h as f64 / film.w as f64 * 0.5).round() as usize;
...
pub async fn run(loops: u32, forever: bool, follow: Option<std::path::PathBuf>) -> Result<()> {
    ...
    let film = load()?;
    let logo = load_logo()?;
    let delay = std::time::Duration::from_millis((1000.0 / (film.fps.max(1) as f64 * 1.5)) as u64);
        ...
        for i in 0..film.frames {
            ...
            if use_kitty || use_virtual {
                ...
                if use_virtual {
                    print!(
                        "{}",
                        render_kitty_virtual(film, payload, cols, rows, &status)
                    );
                } else {
...
mod tests {
    ...
    fn renders_within_bounds() {
        ...
        assert!(s.contains('▀'));
        for line in s.lines() {
            let visible = line.chars().filter(|c| *c == '▀' || *c == ' ').count();
            assert!(visible <= 60);
        }
    }
}
...
fn kitty_virtual_logo(cols: usize) -> String {
    ...
    let mut out = kitty_transmit(&format!("a=T,U=1,f=100,t=d,i=42,c={c},r={rows},q=2"), &b64);
