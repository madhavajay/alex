...
| `repack plan` / `apply` / `resume RUN_ID` | Select sealed chunk-only body packs by `--min-garbage-bytes` and `--min-garbage-ratio`, copy only currently reachable chunks, verify the replacement, atomically switch catalog locations, and move the source to recoverable quarantine. Combined canonical-record packs are safely excluded pending a graph-preserving rewrite. Reports distinguish logical from physical reclamation. |
