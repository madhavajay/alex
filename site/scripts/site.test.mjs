...
test("landing page contains the requested sections and links", async () => {
  ...
  assert.match(html, /rel="canonical" href="https:\/\/madhavajay\.github\.io\/alex\/"/);
  assert.match(html, /property="og:title" content="Alex LLM Proxy — Your tokens, your choice"/);
  assert.match(html, /property="og:description" content="Use any AI subscription in any harness, route and fail over with customizable middleware, and inspect every trace\."/);
  assert.match(html, /property="og:image" content="https:\/\/raw\.githubusercontent\.com\/madhavajay\/alex\/main\/images\/header\.jpg"/);
  assert.match(html, /property="og:image:width" content="1680"/);
  assert.match(html, /property="og:image:height" content="945"/);
  assert.match(html, /name="twitter:card" content="summary_large_image"/);
  assert.match(html, /name="twitter:title" content="Alex LLM Proxy — Your tokens, your choice"/);
  assert.match(html, /name="twitter:description" content="Use any AI subscription in any harness, route and fail over with customizable middleware, and inspect every trace\."/);
  assert.match(html, /name="twitter:image" content="https:\/\/raw\.githubusercontent\.com\/madhavajay\/alex\/main\/images\/header\.jpg"/);
  ...
  assert.match(html, /styles\.css\?v=20260724-responsive-player/);
...
test("site styles preserve the Alex application design language", async () => {
  ...
  assert.match(css, /@media \(min-width: 860px\)[\s\S]*?\.player-panel cove-player \.cb-stage/);
  assert.match(css, /@media \(max-width: 859px\)[\s\S]*?\.player-panel cove-player \.cb-rail[\s\S]*?grid-row: 2/);
  assert.match(css, /\.segmented-control button\[aria-selected="true"\][\s\S]*?border-color: rgba\(10, 132, 255, 0\.58\)/);
